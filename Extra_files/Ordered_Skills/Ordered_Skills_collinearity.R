...
cor_table[which(cor_table>0.90)]
which(abs(cor_table)>0.80,arr.ind = T)
cor_table
corrplot(X_home_diff)
...
# correlations checking
 cor(data_by_sets$sets_difference,dataList$X["failed_passes"])
cor(data_by_sets$sets_difference,dataList$X["perfect_serves"])
cor(data_by_sets$sets_difference,dataList$X["very_good_serves"])
cor(data_by_sets$sets_difference,dataList$X["poor_passes"])
cor(data_by_sets$sets_difference,dataList$X["perfect_blocks"])
cor(data_by_sets$sets_difference,dataList$X["blocked_att1"])
...
xtable(vif_table_full)
vif(mfull)

#-- By removing both failed and poor passes
mfull<-lm(dataList.Y~.-failed_passes-poor_passes-blocked_att1,data=data_full)
vif(mfull)
round(vif(mfull),1)
...
# Collinearity checking in the GVS-based selected covariates
# c(
#   "perfect_serves","very_good_serves",
#   "failed_serves","perfect_passes","very_good_passes",
#   "poor_passes","failed_passes","perfect_att1",
...
mfull<-lm(dataList.Y~failed_serves+failed_passes+perfect_att1+
            failed_att1+perfect_att2+failed_att2+
            perfect_blocks+failed_settings,data=data_full)
vif(mfull)
round(vif(mfull),1)
vif_table_full<-as.matrix(round(vif(mfull),1)) 
xtable(vif_table_full)
