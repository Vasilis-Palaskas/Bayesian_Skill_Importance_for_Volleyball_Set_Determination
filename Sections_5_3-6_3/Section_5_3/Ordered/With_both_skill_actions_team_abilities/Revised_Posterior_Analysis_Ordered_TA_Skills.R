...
X_ordered_Skills<-skill_events[,colnames(skill_events)%in%
                                 c("perfect_serves","failed_serves",
                                   ...
                                   "failed_settings") ]
model_data<-list(Y=data_by_sets$sets_difference_factor,X=X_home_diff,
...
setwd("C:/Users/vasileios palaskas/Desktop/Github folder/Bayesian_Variable_Selection_Volleyball/Ordered_TA_Skills/Revised_Analysis")

ordered_skills_after_BVS_model<-stan("Ordered_TA_Skills_after_BVS.stan",iter=14000, warmup=4000,chains=2,thin=2,
                                     data=model_data,control=list(max_treedepth=15),cores=2)


save(ordered_skills_after_BVS_model,file="revised_ordered_TA_skills_after_BVS")
...
waic(log_lik_ordered_skills_after_BVS_model)####280.9 24.6
loo(log_lik_ordered_skills_after_BVS_model,
    r_eff=r_eff_log_lik_ordered_skills_after_BVS_model)#  286.4 (25.0)
DIC_Gelman(deviance_ordered_skills_after_BVS_model)# 262.6
